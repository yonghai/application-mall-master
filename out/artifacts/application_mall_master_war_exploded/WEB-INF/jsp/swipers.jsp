                        ...
                        <div class="card-box table-responsive">
                            ...
                            <table id="datatable-buttons" class="table table-striped table-bordered table-centered" cellspacing="0" width="100%">
                                ...
                                <tr>
                                    ...
                                    <th style="vertical-align: middle;">更新时间</th>
                                    <th style="vertical-align: middle;">发布状态</th>
		...
		<script type="text/javascript">
			...
			$(document).ready(function() {
                ...
                table = $('#datatable-buttons').DataTable({
                    ...
                    ajax: function (data, callback, settings) {
		                ...
		                $.ajax({
		                    type: "POST",
		                    url: "<c:url value="/swiper/listPage"/>",
		                    cache: false,  //禁用缓存
		                    data: {"swiperCustom.limit":param.limit,"swiperCustom.start":param.start,"swiperCustom.page":param.page}, //传入组装的参数
		            ...
		            columns: [
		                ...
		                { 
		                	...
		                	"render": function ( data, type, full, meta ) {  
			                    return '<div><a href="'+full.url+'" data-lightbox="gallery" data-title="'+full.updatetime+'"><img height="50px" class="picture-thumb" src="'+data+'"></a></div>';  
			                } 
						},
		                { "data": "updatetime" },
		                { 
		                	...
		                	"render": function ( data, type, full, meta ) {
		                		var _html;
		                		if(eval(0) == eval(data)){
		                			_html='<span class="label label-success radius">草稿</span>';
		                		}else if(eval(1) == eval(data)){
		                			_html='<span class="label label-success radius">已上线</span>';
		                		}else if(eval(2) == eval(data)){
		                			_html='<span class="label label-defaunt radius">已下架</span>';
		                		}else{
		                			_html='异常';
		                		}
						...
						{ 
		                	...
		                	"render": function ( data, type, full, meta ) {  
			                    ...
			                    _html = _html + '<a class="ml-2" onClick="edit(\'编辑\',\'<c:url value="/swiper/select"/>\',\''+data+'\')" href="javascript:;" title="编辑"><i class="dripicons-pencil"></i></a>';
			                    _html = _html + '<a class="ml-2" onClick="del(this,\''+data+'\',\''+full.url+'\')" href="javascript:;" title="删除"><i class="dripicons-trash"></i></a>';
			                    return _html;  
			                } 
						}
		            ]
			...
			function del(obj,id,url){
				layer.confirm('确认要删除吗？',function(index){
					$.ajax({
						url: '<c:url value='/swiper/delete'/>',
						type : 'post',
						data: {"swiperCustom.id":id,"swiperCustom.url":url},
			         	dataType: "json",
			...
			function unpublished(obj,id){
				layer.confirm('确认要下架吗？',function(index){
					$.ajax({
						url: '<c:url value='/swiper/xiajia'/>',
						type : 'post',
						data: {"swiperCustom.id":id},
			         	dataType: "json",
						success : function(data) {
							if(data.ok){
								var _id = "'"+id+"'";
