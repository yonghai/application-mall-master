...
<html>
    ...
    <body>
                        ...
                        <div class="card-box table-responsive">
                            ...
                            <table id="datatable-buttons" class="table table-striped table-bordered table-centered" cellspacing="0" width="100%">
                                ...
                                <tr>
                                    ...
                                    <th style="vertical-align: middle;">用户</th>
                                    <th style="vertical-align: middle;">性别</th>
                                    <th style="vertical-align: middle;">城市</th>
                                    <th style="vertical-align: middle;">省份</th>
		...
		<script type="text/javascript">
			...
			$(document).ready(function() {
                ...
                table = $('#datatable-buttons').DataTable({
                    ...
                    ajax: function (data, callback, settings) {
		                ...
		                $.ajax({
		                    type: "POST",
		                    url: "<c:url value="/user/wxUserList"/>",
		                    cache: false,  //禁用缓存
		                    data: {"wxUserCustom.limit":param.limit,"wxUserCustom.start":param.start,"wxUserCustom.page":param.page}, //传入组装的参数
		                    dataType: "json",
		            ...
		            columns: [
						...
						{ 
		                	...
		                	"render": function ( data, type, full, meta ) {
		                		var _html = '';
		                		if(eval(1) == eval(full.state)){
		                			_html='<a onClick="jiejin(this,\''+data+'\')" href="javascript:;" title="解禁"><i class="dripicons-direction"></i></a> ';
		                		}else if(eval(0) == eval(full.state)){
		                			_html='<a onClick="jinyong(this,\''+data+'\')" href="javascript:;" title="禁用"><i class="dripicons-skip"></i></a>';
		                		}
			                    _html = _html + '<a class="ml-2" onClick="del(this,\''+full.id+'\')" href="javascript:;" title="删除"><i class="dripicons-trash"></i></a>';
			                    return _html;  
			                } 
						}
		            ]
			...
			/*解禁*/
			function jiejin(obj,id){
				layer.confirm('确认要解禁该用户吗？',function(index){
					$.ajax({
						url: '<c:url value='/user/changeState'/>',
						type : 'post',
						data: {"state":0,"id":id},
			         	dataType: "json",
						success : function(data) {
							var _id = "'"+id+"'";
							if(data.code === 1){
								var html_text = '<a onClick="jinyong(this,'+_id+')" href="javascript:;" title="禁用"><i class="dripicons-skip"></i></a>';
								$(obj).parents("tr").find(".td-manage").prepend(html_text);
								$(obj).parents("tr").find(".td-status").html('<span class="label label-defaunt radius">正常</span>');
								$(obj).remove();
								layer.msg('已解禁该用户!',{icon: 6,time:1000});
							}else{
								layer.msg('操作失败!'+data.result,{icon: 5,time:1000});
						    }
						},
				...
				layer.confirm('确认要禁用该用户吗？',function(index){
					$.ajax({
						...
						success : function(data) {
							var _id = "'"+id+"'";
							if(data.code === 1){
								var html_text = '<a onClick="jiejin(this,'+_id+')" href="javascript:;" title="解禁"><i class="dripicons-direction"></i></a>';
								$(obj).parents("tr").find(".td-manage").prepend(html_text);
								$(obj).parents("tr").find(".td-status").html('<span class="label label-defaunt radius">禁用</span>');
								$(obj).remove();
								layer.msg('已禁用该用户!',{icon: 6,time:1000});
							}else{
								layer.msg('操作失败!'+data.result,{icon: 5,time:1000});
						    }
						},
