    ...
    <body>
            ...
            <div class="container-fluid">
                        ...
                        <div class="page-title-box">
                            <div class="btn-group pull-right">
                                <ol class="breadcrumb hide-phone p-0 m-0">
                                    ...
                                    <li class="breadcrumb-item active">新增属性</li>
                                </ol>
                            </div>
                ...
                <div class="row">
                    <div class="col-sm-6" style="margin: auto;">
                        ...
                        <div class="card-box">
                            <h4 class="header-title m-t-0">新增属性</h4>
                            <p class="text-muted font-14 m-b-20">
                                新增一个属性，在增加商品时可以添加该属性。
                            </p>

                            <form id="formSubmitAction" role="form">
                            	<input type="hidden" id="token" name="token" value="${token }">
                                <div class="form-group">
                                    <label for="name">属性名称<span class="text-danger">*</span></label>
                                    <input type="text" name="name" parsley-trigger="change" required
                                           placeholder="输入属性名称" class="form-control" id="name">
                                </div>
                                <div class="form-group">
		...
		<script type="text/javascript">
			$('#formSubmitAction').submit(function() {
				var name = $('#name').val().trim();//标题
				var token = $('#token').val().trim();//标题
				var _options = $("#select").find("option"); // select下所有的option
				var options = '';
				for (var i=0; i<_options.length; i++) {
				    options+=_options.eq(i).val()+',';
				}
				if(options.length < 1){
					layer.msg('属性不能为空');
					return false;
				}
				layer.confirm('提交后不可更改,是否继续？', {
				  btn: ['是','否'] //按钮
				}, function(){
				  $.ajax({
					url: '<c:url value='/property/save'/>',
					type : 'post',
					data: {"propertyCustom.name":name,options:options,"token":token},
	          		dataType: "json",
	          		beforeSend:function(){  
	                    _index = layer.load(2);
	                },
